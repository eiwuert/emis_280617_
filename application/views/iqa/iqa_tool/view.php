                            ...
                            <div class="box box-solid box-info col-xs-12 col-lg-12 no-padding">
                                ...
                                <div class="form-group col-xs-12 col-lg-12 col-lg-12">
                                    <div class="col-sm-4 col-xs-12">
                                        <?php echo form_label(lang('iqa_evaluation_for'). ':', 'iqa_evaluate_for', array('class' => 'col-sm-6 col-xs-12 required no-padding')); ?>
                                        <span class="input-group date col-sm-12 col-xs-12">
                                            <?php echo form_dropdown('evaluate_for', $dropEvlFor, $iqa_result_info->evaluate_for, 'class="form-control evaluate_for"');?>
                                            <?php echo form_hidden('edit_evaluate_for',$iqa_result_info->evaluate_for)?>
                                        </span>
                                    </div>
                                    <div class="col-sm-4 col-xs-12">
                                        <?php echo form_label(lang('iqa_employee_name'). ':', 'employee_name', array('class' => 'col-sm-6 col-xs-12 required no-padding')); ?>
                                        <span class="input-group date col-sm-12 col-xs-12">
                                            <?php echo form_dropdown('employee', $employees_somu, (($iqa_result_info->evaluate_to != "")? $iqa_result_info->evaluate_to : ''), 'class="form-control"');?>
                                        </span>
                                    </div>
                                    <div class="col-sm-4 col-xs-12" style="margin-bottom:5px;" id="disAccessor">
                                    ...
                                    <div class="col-sm-4 col-xs-12" style="margin-bottom:5px;" id="disRoom">
                                        <?php echo form_label(lang('iqa_result_accessor'). ':', array('class' => 'col-sm-6 col-xs-12 required no-padding')); ?>
                                        <?php
                                            $selected_accessor = explode(",", $iqa_result_info->evaluate_by);
                                            echo form_dropdown(
                                                'accessor_by_room[]',
                                                $accessor_byroom,
                                                $info_evaluate_by,
                                                'class="form-control" multiple="multiple" id="accessorByRoom"'
                                            );
                                        ?>
                                    </div>
                                    <div class="col-sm-4 col-xs-12">
                                    ...
                                    <div class="col-sm-4 col-xs-12" style="margin-bottom:1px">
...
<script type="text/javascript">
    ...
    {
        // // check accessor
        $('#disAccessor').css('display','none');
        $('#disRoom').css('display','block');
        ...
        initSomuSelect('#accessorByRoom', false);
        var edit_evaluate_for = $("input[name='edit_evaluate_for']").val();
        // // check edit
        if(edit_evaluate_for == 1){
            $('#disAccessor').css('display','none');
            $('#disRoom').css('display','block');
            $('.evaluate_for').prop( "disabled", true );
        }
        if(edit_evaluate_for == 2){
            $('#disAccessor').css('display','block');
            $('#disRoom').css('display','none');
            $('.evaluate_for').prop( "disabled", true );
        }

        $('.evaluate_for').change(function(){
            if($(this).val() == 1){
                $('#disAccessor').css('display','none');
                $('#disRoom').css('display','block');
            }
