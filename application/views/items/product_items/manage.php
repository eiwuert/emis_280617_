...
<div class="main-content-inner">
    <div class=" alert alert-info" id='top'>
        <?php echo create_breadcrumb(); ?></div>
        ...
        <div class=" pull-right">
            <div class="row">
                <div class="col-md-12 center" style="text-align: center;">  
                        ...
                        <div class="discount_selection pull-left" style="margin-right:5px">
                            <?php echo form_dropdown('unitType',$unitType,'','class="ui-autocomplete-input" id="searchUnitType"')?>
                            <?php echo form_hidden('unitType',$unitType,'')?>
                            <?php echo form_input('discount','','class="discount" placeholder="% 100"')?>
                        ...
                        <?php 
                            ...
                            <?php echo anchor("$controller_name/view/-1/", '<i title="' . lang($controller_name . '_new') . '" class="fa fa-pencil tip-bottom hidden-lg fa fa-2x"></i><span class="visible-lg">' . lang($controller_name . '_new') . '</span>', array('id' => 'new-person-btn', 'class' => 'btn btn-success', 'title' => $this->lang->line($controller_name . '_new')));
                            }
                        ?>
            ...
            <div class="col-xs-30">
                <div class="widget-box" id="widgets">
                    <div class="widget-title">
                        ...
                        <a href="<?php echo site_url($controller_name.'/clear_state'); ?>" class="clear-state pull-right">
                            <span class="label label-info tip-left"> <?php echo lang('common_clear_search'); ?> </span>
                        </a>
                    </div>
                    <div class="widget-content nopadding table_holder table-responsive manage_table" >
                        <?php echo $manage_table; ?>
                    </div>
                    <div id="manage_pagination">
                    <?php if ($pagination) { ?>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $("#searchUnitType").on('change',function(){            
            var id = $('#searchUnitType option:selected').val();
            $.post("<?php echo site_url("$controller_name/search_typeUnit")?>", {id: id}, function(result){
                $('#bodyDrop').html(result.manage_table);
                $('#manage_pagination').html(result.pagination);                
            },"json");
        });

        $("#submit").on('click',function(){
            var row_ids = get_selected_values().toString();
            var searchUnitType = $('#searchUnitType').val();
            var disocunt = $('.discount').val();

            $.post("<?php echo site_url("$controller_name/discount_unit")?>", {row_ids: row_ids, searchUnitType:searchUnitType, disocunt:disocunt}, function(response){
                if (response.success)
                {
