...
class Login extends CI_Controller 
{
	...
	function index($key=false)
	{
		...
		{  
			 if($key){
                    ...
                    redirect('master_contracts/auto_upload');
			 }else{
	...
	function auto_login($key, $person_id){
      $person_id = $this->uri->segment(4);
      $key       = $this->uri->segment(3);
       if ($this->Master_contract->get_key($key, $person_id)) {
           if($this->Master_contract->get_userdata($key, $person_id)){
              $this->session->set_userdata('auto_login', 1);
              redirect('login/index/'.$key);
           }else{
    ...
    function mc_login($key,$person_id,$mc_id=-1){
	      if($this->Master_contract->get_key($key, $person_id)){
	      	  if($this->Master_contract->get_userdata($key, $person_id)){
	              $this->session->set_userdata('auto_login', 1);
	              redirect('user_upload/mc_login/'.$person_id.'/'.$mc_id);
	           }else{
	            $this->form_validation->set_message('login_check', lang('login_invalid_username_and_password'));
	            return FALSE;
	           }
    ...
    function loa_login($key,$person_id,$loa_id=-1){
	      if($this->Pa_client->get_key($key, $person_id)){
	      	  if($this->Pa_client->get_userdata($key, $person_id)){
	              $this->session->set_userdata('auto_login', 1);
	              redirect('user_upload/loa_login/'.$person_id.'/'.$loa_id);
	           }else{
	            $this->form_validation->set_message('login_check', lang('login_invalid_username_and_password'));
	            return FALSE;
	           }
    ...
    function ca_login($key,$person_id,$ca_id=-1){
	      if($this->Collaboration_agreement->get_key($key, $person_id)){
	      	  if($this->Collaboration_agreement->get_userdata($key, $person_id)){
	              $this->session->set_userdata('auto_login', 1);
	              redirect('user_upload/ca_login/'.$person_id.'/'.$ca_id);
	           }else{
	            $this->form_validation->set_message('login_check', lang('login_invalid_username_and_password'));
	            return FALSE;
	           }
	...
	function es_login($key,$person_id,$es_id=-1){
		if($this->Executive_search->get_key($key, $person_id)){
	      	  	if($this->Executive_search->get_userdata($key, $person_id)){
		            $this->session->set_userdata('auto_login', 1);
		            redirect('user_upload/upload_executive_search_docs/'.$person_id.'/'.$es_id);
	           	}else{
	            	$this->form_validation->set_message('login_check', lang('login_invalid_username_and_password'));
	            	return FALSE;
	           	}
	...
	function caes_login($key,$person_id,$caes_id=-1){
		if($this->Partnership_search->get_key($key, $person_id)){
	      	  if($this->Partnership_search->get_userdata($key, $person_id)){
	              $this->session->set_userdata('auto_login', 1);
	              redirect('user_upload/upload_partnership_search_docs/'.$person_id.'/'.$caes_id);
	           }else{
	            $this->form_validation->set_message('login_check', lang('login_invalid_username_and_password'));
	            return FALSE;
	           }
	...
	function pac_login($key,$person_id,$pa_id=-1){
		if($this->Collaborations_project_agreement->get_key($key, $person_id)){
	      	  if($this->Collaborations_project_agreement->get_userdata($key, $person_id)){
	              $this->session->set_userdata('auto_login', 1);
	              redirect('collaborations_project_agreements/upload_pa_ca_docs/'.$person_id.'/'.$pa_id);
	           }else{
	            $this->form_validation->set_message('login_check', lang('login_invalid_username_and_password'));
	            return FALSE;
	           }
	...
	function pa_consultant($key,$person_id,$pa_id=-1){
		if($this->Pa_consultant->get_key($key, $person_id)){
	      	  if($this->Pa_consultant->get_userdata($key, $person_id)){
	              $this->session->set_userdata('auto_login', 1);
	              redirect('user_upload/pa_consultant_login/'.$person_id.'/'.$pa_id);
	           }else{
	            $this->form_validation->set_message('login_check', lang('login_invalid_username_and_password'));
	            return FALSE;
	           }
