...
{	
			...
			if ($this->input->get('w') != '') { // From where should we return data
				if ($this->input->get('term') != '') { // What exactly are we searchin
					...
					switch($this->input->get('w')) {
						...
						break;
						case 'proposals': 
							$t = $this->Proposal->search($this->input->get('term'), 100, 0)->result_object();
							$tmp = array();
							foreach ($t as $k=>$v) { $tmp[$k] = array('id'=>$v->proID, 'name'=>$v->title.", ".$v->proposal_code); }
							die(json_encode($tmp));
						break;
						case 'clients': 
							$t = $this->Customer->search($this->input->get('term'), 100, 0, 'last_name', 'asc')->result_object();
							$tmp = array();
							foreach ($t as $k=>$v) { $tmp[$k] = array('id'=>$v->person_id, 'name'=>$v->company_name." - ".$v->email.' - '.$v->person_id); }
							die(json_encode($tmp));
						break;
						case 'project_managers': 
							$t = $this->Employee->search_by_user_type($this->input->get('term'), 100, 0, 'last_name', 'asc', array(9))->result_object();
							$tmp = array();
							foreach ($t as $k=>$v) { $tmp[$k] = array('id'=>$v->person_id, 'name'=>$v->full_name." - ".$v->email); }
							die(json_encode($tmp));
						break;
						case 'project_titles': 
							$t = $this->Proposal->search_project_titles($this->input->get('term'), 100, 0)->result_object();
							$tmp = array();
							foreach ($t as $k=>$v) { $tmp[$k] = array('id'=>$v->projID, 'name'=>$v->project_title.", ".$v->project_id); }
							die(json_encode($tmp));
						break;
						case 'consultants': 
							$t = $this->Employee->search_by_user_type($this->input->get('term'), 100, 0, 'last_name', 'asc', array(4))->result_object();
							$tmp = array();
							foreach ($t as $k=>$v) { $tmp[$k] = array('id'=>$v->person_id, 'name'=>$v->full_name." - ".$v->email.' >'.$v->person_id); }
							die(json_encode($tmp));
						break;
						case 'responsible_person': 
							$t = $this->Employee->search_by_user_type($this->input->get('term'), 100, 0, 'last_name', 'asc', array(4, 2, 8, 9, 1))->result_object();
							$tmp = array();
							foreach ($t as $k=>$v) { $tmp[$k] = array('id'=>$v->person_id, 'name'=>$v->full_name." - ".$v->email); }
							die(json_encode($tmp));
						break;
    ...
    //reaksmey code.............   
    function proposal_input_excel_export() {
        $data = $this->_get_common_report_data(TRUE);
        $this->load->view("reports/proposal_input_excel_export", $data);
    }
    
    function expense_input_excel_export() {
        $data = $this->_get_common_report_data(TRUE);
        $this->load->view("reports/expense_input_excel_export", $data);
    }
	
	function profit_loss_input_excel_export() {
        $data = $this->_get_common_report_data(TRUE);
        $this->load->view("reports/tabular_details_profit_lost", $data);
    }
    
    function claim_input_excel_export() {
        $data = $this->_get_common_report_data(TRUE);
        $this->load->view("reports/claim_input_excel_export", $data);
    }
       
    //proposal report.....
    function summary_proposal($start_date, $end_date, $status, $approval, $export_excel = 0, $proposal_ids='all', $project_manager_ids='all', $client_ids='all') {
        ...
        if ($proposal_ids != 'all') {
        	$proposal_ids = explode("-",$proposal_ids);
        }
        if ($project_manager_ids != 'all') {
        	$project_manager_ids = explode("-",$project_manager_ids);
        }
        if ($client_ids != 'all') {
        ...
        $model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'export_excel' => $export_excel, 'status' => $status, 'approval' => $approval, 'proposal_ids' => $proposal_ids, 'project_manager_ids' => $project_manager_ids, 'client_ids' => $client_ids));

        $config                = array();
        $config['base_url']    = site_url("reports/Summary_proposal/" . rawurlencode($start_date) . '/' . rawurlencode($end_date) . "/$status/$approval/$export_excel");
        $config['total_rows']  = $model->getTotalRows();
        $config['per_page']    = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        $config['uri_segment'] = 7;
        $this->pagination->initialize($config);
        ...
        foreach ($report_data as $row) {
        	$person_info = $this->Employee->get_info($row['resp_person_id']);
        	$person_name = $person_info->full_name;
            $tabular_data[] = array(
                array('data'  => $row['proposal_code'], 'align' => 'left'),
                array('data'  => $row['proposal_title'], 'align' => 'left'),
                array('data'  => H($person_name), 'align' => 'left'),
                array('data'  => date('D, d-m-Y', strtotime($row['created_date'])), 'align' => 'left'),
                array('data'  => $row['company_name'], 'align' => 'left'),
                array('data'  => $row['subject'], 'align' => 'left'),
                array('data'  => $row['pro_status'], 'align' => 'left'),
                array('data'  => '<a target="_blank" href="'.site_url().'proposals/pre_preview/'.$row['pro_id'].'">'.lang('reports_view').'</a>', 'align' => 'left'),
            );
        }

        $data = array(
            "title"        => lang('reports_proposal_report'),
        ...
        $this->load->view("reports/tabular_proposal", $data);
        
    }
    //expense report.....
    function summary_expense($start_date, $end_date, $export_excel = 0, $project_ids='all', $project_manager_ids='all') {
        $this->check_action_permission('view_expense');
        $start_date = rawurldecode($start_date);
        $end_date   = rawurldecode($end_date);
        ...
        $model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'export_excel' => $export_excel, 'project_ids' => $project_ids, 'project_manager_ids' => $project_manager_ids));

        $config                = array();
        $config['base_url']    = site_url("reports/summary_expense/" . rawurlencode($start_date) . '/' . rawurlencode($end_date) . "/$export_excel/");
        $config['total_rows']  = $model->getTotalRows();
        $config['per_page']    = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        $config['uri_segment'] = 7;
        $this->pagination->initialize($config);
        ...
        $i            = 1;
        foreach ($report_data['summary'] as $key => $row) {
        	$person_info = $this->Employee->get_info($row['project_manager_id']);
        	$person_name = $person_info->full_name;
            $summary_data_row = array();

            $summary_data_row[] = array('data'  => $i, 'align' => 'left');
            $summary_data_row[] = array('data'  => date('d-m-Y', strtotime($row['issue_date'])), 'align' => 'left');
            $summary_data_row[] = array('data'  => H($row['project_title']), 'align' => 'left');
            $summary_data_row[] = array('data'  => H($person_name), 'align' => 'left');
            $summary_data_row[] = array('data'  => to_currency_no_money($row['sub_total']), 'align' => 'left');
            $summary_data_row[] = array('data'  => anchor('project_expense/view/'.$row['expencesID'], lang('reports_view'), array('target' => '_blank')), 'align' => 'left');
            ...
            foreach ($report_data['details'][$key] as $drow) {
                ...
                $details_data_row[]   = array('data'  => date('d-m-Y', strtotime($drow['item_date'])), 'align' => 'left');
                $details_data_row[]   = array('data'  => H($drow['item_name']), 'align' => 'left');
                $details_data_row[]   = array('data'  => H($drow['expence_name']), 'align' => 'left');
                $details_data_row[]   = array('data'  => to_quantity($drow['quantity']), 'align' => 'left');
                $details_data_row[]   = array('data'  => '('.$drow['currency'].') '.to_currency_no_money($drow['unit_price']), 'align' => 'left');
                $details_data_row[]   = array('data'  => number_format((float)$drow['rate'], 2, '.', ''), 'align' => 'left');
                $details_data_row[]   = array('data'  => to_currency_no_money($drow['sub_total']), 'align' => 'left');
                $details_data_row[]   = array('data'  => H($drow['employee_name']), 'align' => 'left');
        ...
        $data = array(
            "title"                => lang('reports_expense_report'),
            "subtitle"             => date('D, d-m-Y', strtotime($start_date)) . ' - ' . date('D, d-m-Y', strtotime($end_date)),
            "headers"              => $model->getDataColumns(),
            "summary_data"         => $summary_data,
            "details_data"         => $details_data,
        ...
        $this->load->view("reports/tabular_details_expense", $data);
        
    }
    //claim report.....
    function summary_claim($start_date, $end_date, $export_excel = 0, $project_ids='all', $project_manager_ids='all', $consultants='all') {
        $this->check_action_permission('view_claim');
        $start_date = rawurldecode($start_date);
        $end_date   = rawurldecode($end_date);
        ...
        if ($consultants != 'all') {
        	$consultants = explode("-",$consultants);
        }

        $this->load->model('reports/Summary_claims');
        $model = $this->Summary_claims;

        $model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'export_excel' => $export_excel, 'project_ids' => $project_ids, 'project_manager_ids' => $project_manager_ids, 'consultants' => $consultants));

        $config                = array();
        $config['base_url']    = site_url("reports/summary_claim/" . rawurlencode($start_date) . '/' . rawurlencode($end_date) . "/$export_excel/$project_ids/$project_manager_ids/$consultants");
        $config['total_rows']  = $model->getTotalRows();
        $config['per_page']    = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        $config['uri_segment'] = 7;
        $this->pagination->initialize($config);
        ...
        foreach ($report_data['summary'] as $key => $row) {
            ...
            $summary_data_row = array();
            $summary_data_row[] = array('data'  => $row['project_title'], 'align' => 'left');
            $summary_data_row[] = array('data'  => date('d-m-Y', strtotime($row['claim_date'])), 'align' => 'left');
            $summary_data_row[] = array('data'  => H($person_name), 'align' => 'left');
            $summary_data_row[] = array('data'  => to_currency_no_money($row['sub_total']), 'align' => 'left');
            $summary_data_row[] = array('data'  => anchor('project_claim/view/'.$row['claim_id'], lang('reports_view'), array('target' => '_blank')), 'align' => 'left');
        ...
        $data = array(
            "title"                => lang('reports_claim_report'),
            "subtitle"             => date('D, d-m-Y', strtotime($start_date)) . ' - ' . date('D, d-m-Y', strtotime($end_date)),
            "headers"              => $model->getDataColumns(),
            "summary_data"         => $summary_data,
            "details_data"         => $details_data,
        ...
        $this->load->view("reports/tabular_details_claim", $data);
        
    }
	
	//detail profit and loss
	function detailed_profit_and_loss($start_date, $end_date, $export_excel = 0, $project_ids='all', $project_manager_ids='all', $client_ids='all') {
        $this->check_action_permission('view_profit_and_loss');
        $start_date = rawurldecode($start_date);
        $end_date   = rawurldecode($end_date);
        ...
        $this->load->model('reports/Detail_profit_loss');
        $model = $this->Detail_profit_loss;
                                      
        $model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'export_excel' => $export_excel, 'project_ids' => $project_ids));

        $config                = array();
        $config['base_url']    = site_url("reports/detailed_profit_and_loss/" . rawurlencode($start_date) . '/' . rawurlencode($end_date) . "/$export_excel/");
        $config['total_rows']  = $model->getTotalRows();
        $config['per_page']    = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        $config['uri_segment'] = 7;
        $this->pagination->initialize($config);
        ...
        foreach ($report_data['summary'] as $key => $row) {
            ...
            $summary_data_row[] = array('data'  => $i, 'align' => 'left');
            $summary_data_row[] = array('data'  => $row['project_title'], 'align' => 'left');
            $summary_data_row[] = array('data'  => date('D, d-M-Y', strtotime($row['start_date'])), 'align' => 'left');
            $summary_data_row[] = array('data'  => date('D, d-M-Y', strtotime($row['end_date'])), 'align' => 'left');
            $summary_data_row[] = array('data'  => to_currency($row['inv_amount']), 'align' => 'left');
            $summary_data_row[] = array('data'  => to_currency($row['claim_amount']), 'align' => 'left');
            $summary_data_row[] = array('data'  => to_currency($row['expense_amount']), 'align' => 'left');
            $summary_data_row[] = array('data'  => to_currency($row['inv_amount']-($row['claim_amount']+$row['expense_amount'])), 'align' => 'left');
            $summary_data[$key] = $summary_data_row;
			$j = 1;
			$revenues = $model->get_revenue($row['project_id']);
			
            foreach ($revenues->result() as $drow) {
                $details_data_row = array();

                $details_data_row[]   = array('data'  => $j, 'align' => 'left');
                $details_data_row[]   = array('data'  => date('D, d-m-Y', strtotime($drow->inv_date)), 'align' => 'left');
                $details_data_row[]   = array('data'  => $drow->description, 'align' => 'left');
                $details_data_row[]   = array('data'  => to_currency($drow->inv_amount), 'align' => 'left');
				$details_data[$key][] = $details_data_row;
                $j++;
            }
			$a=1;
			$expenses = $model->get_expenses($row['project_id']);
			foreach ($expenses->result() as $drow1) {
                $details_expens_row = array();

                $details_expens_row[]   = array('data'  => $a, 'align' => 'left');
                $details_expens_row[]   = array('data'  => date('D, d-m-Y', strtotime($drow1->expense_date)), 'align' => 'left');
                $details_expens_row[]   = array('data'  => $drow1->item, 'align' => 'left');
                $details_expens_row[]   = array('data'  => to_currency($drow1->expense_amount), 'align' => 'left');
				$details_expense_data[$key][] = $details_expens_row;
                $a++;
            }
			$b=1;
			$claims = $model->get_claims($row['project_id']);
			
			foreach ($claims->result() as $drow1) {
                $details_claims_row = array();

                $details_claims_row[]   = array('data'  => $b, 'align' => 'left');
                $details_claims_row[]   = array('data'  => date('D, d-m-Y', strtotime($drow1->claim_date)), 'align' => 'left');
                $details_claims_row[]   = array('data'  => $drow1->item, 'align' => 'left');
                $details_claims_row[]   = array('data'  => to_currency($drow1->claim_amount), 'align' => 'left');
				$details_claims_data[$key][] = $details_claims_row;
                $b++;
            }
            $i++;
        }
        ...
        $data = array(
            ...
            "summary_data"         => $summary_data,
            "details_data"         => $details_data,
            "details_claim_data"   => $details_claims_data,
            "details_expense_data" => $details_expense_data,
            "overall_summary_data" => $model->getSummaryData(),
            "export_excel"         => $export_excel,
            "pagination"           => $this->pagination->create_links(),
        );

        $this->load->view("reports/tabular_details_profit_loss", $data);
        
    }
	
	//graphical profit and loss 
	//detail profit and loss
	function graphic_profit_and_loss($start_date, $end_date, $export_excel = 0, $project_ids='all', $project_manager_ids='all', $client_ids='all') {
        $this->check_action_permission('view_profit_and_loss');
        $start_date = rawurldecode($start_date);
        $end_date   = rawurldecode($end_date);
        ...
        foreach ($report_data['summary'] as $key => $row) {
            ...
            $summary_data_row[] = array('data'  => $row['project_title'].'<br/>'.date('d,M,Y',strtotime($row['start_date'])).' to '.date('d,M,Y',strtotime($row['end_date'])), 'align' => 'left');
            $summary_data_row[] = array('data'  => $row['inv_amount'], 'align' => 'left');
            $summary_data_row[] = array('data'  => $row['claim_amount'], 'align' => 'left');
            $summary_data_row[] = array('data'  => $row['expense_amount'], 'align' => 'left');
            $summary_data_row[] = array('data'  => $row['inv_amount']-($row['claim_amount']+$row['expense_amount']), 'align' => 'left');
            $summary_data[$key] = $summary_data_row;
        ...
        $data = array(
            "title"                => lang('reports_expense_report'),
            "subtitle"             => 'Profit & loss from '.date('D, d-m-Y', strtotime($start_date)) . ' to ' . date('D, d-m-Y', strtotime($end_date)),
            "headers"              => $model->getDataColumns(),
            "summary_data"         => $summary_data,
            "details_data"         => $details_data,
            "overall_summary_data" => $model->getSummaryData(),
        ...
        $this->load->view("reports/graphical_profit_and_loss", $data);
