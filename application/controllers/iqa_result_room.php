...
class Iqa_result_room extends Secure_area 
{
    function __construct()
    {
        parent::__construct('iqa_result_room');
    }

	function index($offset = 0, $byRoom = 1)
    {        
		...
		$params = $this->session->userdata('iqa_result_room_search_data') ? $this->session->userdata('iqa_result_room_search_data') : array('offset' => 0, 'order_col' => 'id', 'order_dir' => 'desc', 'search' => FALSE, 'search_frm' => $search_frm);
        if ($offset != $params['offset']) {
            redirect('iqa_result_room/index/'.$params['offset']);
        }
        $config['base_url'] = site_url('iqa_result_room/sorting');
        $config['per_page'] = $this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20;
        $data['controller_name'] = strtolower(get_class());
        $data['per_page'] = $config['per_page'];
        $data['search'] = $params['search'] ? $params['search'] : "";
        if ($data['search']) {
            $config['total_rows'] = $this->Iqa_results->search_count_all($data['search'], $params['search_frm'],$byRoom);
            $table_data = $this->Iqa_results->search($data['search'], $params['search_frm'], $data['per_page'], $params['offset'], $params['order_col'], $params['order_dir'],$byRoom);
        }else{
            $config['total_rows'] = $this->Iqa_results->count_all_byRoom($byRoom);
            $table_data = $this->Iqa_results->get_all_byRoom($data['per_page'], $params['offset'], $params['order_col'], $params['order_dir'],$byRoom);
        }
        $data['iqa_types'] = $this->opt_iqa_types();
        ...
        $data['manage_table'] = get_iqa_result_room_manage_table($table_data, $this);

		$this->load->view('iqa/iqa_result/manage', $data);
	}

    function sorting($byRoom = 1)
    {
        $this->check_action_permission('search');
        ...
        $iqa_result_room_search_data = array('offset' => $offset, 'order_col' => $order_col, 'order_dir' => $order_dir, 'search' => $search, 'search_frm' => $search_frm);
        $this->session->set_userdata("iqa_result_room_search_data", $iqa_result_room_search_data);

        if ($search) {
            $config['total_rows'] = $this->Iqa_results->search_count_all($search, $search_frm,$byRoom);
            $table_data = $this->Iqa_results->search($search, $search_frm, $per_page, $offset, $order_col, $order_dir,$byRoom);
        } else {
            $config['total_rows'] = $this->Iqa_results->count_all($byRoom);
            $table_data = $this->Iqa_results->get_all($per_page, $offset, $order_col, $order_dir,$byRoom);
        }
        $config['base_url'] = site_url('iqa_result_room/sorting');
        $config['per_page'] = $per_page;
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
        $data['manage_table'] = get_iqa_result_room_manage_table_data_rows($table_data, $this);
 ...
 //    {
 //        $this->session->unset_userdata('iqa_result_room_search_data');
 //        redirect('iqa_result');
 //    }
