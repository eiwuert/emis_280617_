...
class Summary_expenses extends Report {
	...
	public function getDataColumns(){
		...
		$return['details'][] = array('data'=>lang('reports_item_expense'), 'align'=> 'left');
	...
	public function getData() {
		...
		$this->db->select('expences.id as expencesID, expences.date as issue_date, project_title, sum(unit_price * quantity * rate) as sub_total, type_price as currency, title as proposal_title, projects.person_incharge_id as project_manager_id');
		...
		$this->db->select('expences.id as expencesID, item_date, item as item_name, quantity as quantity, unit_price, rate, (unit_price * quantity * rate) as sub_total, type_price as currency, claimed_by, full_name as employee_name, expence_name');
        $this->db->from('expences');
        $this->db->join('projects', 'projects.id = expences.project_id', 'left');
        $this->db->join('proposals', 'proposals.id = projects.proposal_id', 'left');
        $this->db->join('expence_details', 'expence_details.expence_id = expences.id', 'left');
        $this->db->join('expence_types', 'expence_details.expence_type_id = expence_types.id', 'left');
        $this->db->join('people', 'people.person_id = expence_details.claimed_by', 'left');
        if ($project_ids != "all") {
            $this->db->or_where_in($this->db->dbprefix('expences') . '.project_id', $project_ids);
        }
	...
	public function getSummaryData()
	{
		...
		$this->db->select('COUNT(DISTINCT('.$this->db->dbprefix('expences').'.id)) as expense_count, sum(unit_price * quantity * rate) as sub_total, projects.id as projectsID, proposals.id as proposalsID',false);
        $this->db->from('expences');
        $this->db->join('projects', 'projects.id = expences.project_id', 'left');
        $this->db->join('proposals', 'proposals.id = projects.proposal_id', 'left');
        $this->db->join('expence_details', 'expence_details.expence_id = expences.id', 'left');
		...
		$return['expense_total'] = number_format((float)$return['expense_total'], 2, '.', '');
		return $return;
	}
