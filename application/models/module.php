...
class Module extends CI_Model 
{
	...
	function get_allowed_modules($person_id)
	{
        ...
        function get_sub_module($key, $person_id) {
                $this->db->from('modules');
                $this->db->join('permissions', 'permissions.module_id=modules.module_id');
                $this->db->where("permissions.person_id", $person_id);
                $this->db->where("modules.sub_module", $key);
                $this->db->order_by("sort", "asc");
                return $this->db->get();
        }
        function get_submodule($id) {
                $this->db->where('sub-module', $id);
                $this->db->order_by("sort", "asc");
                return $this->db->get('modules');
        }
         function get_sub_sub_module($key, $person_id) {
                $this->db->from('modules');
                $this->db->join('permissions', 'permissions.module_id=modules.module_id');
                $this->db->where("permissions.person_id", $person_id);
                $this->db->where("modules.sub2_module", $key);
                $this->db->order_by("sort", "asc");
                return $this->db->get();
        }
