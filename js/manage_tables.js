...
// ==============================
// my view search ditecso
// ==============================

function enable_viewSearch(suggest_url,confirm_search_message)
{

	//Keep track of enable_email has been called
	if(!enable_viewSearch.enabled)
		enable_viewSearch.enabled=true;
	...
	$( "#search" ).autocomplete({
		...
		delay: 300,
 		...
 		{
			$( "#search" ).val(ui.item.label);
			$( "#search_for" ).val(ui.item.unique_id);
			$( "#search_forid" ).val(ui.item.label_id);
 			// do_search(true);
 		}
	});
}
enable_viewSearch.enabled=false;

function enable_codeMajor(suggest_url,confirm_search_message)
{

	//Keep track of enable_email has been called
	if(!enable_codeMajor.enabled)
		enable_codeMajor.enabled=true;

	$('#search_major_code').click(function()
    {
    	$(this).attr('value','');
    });

	$( "#search_major_code" ).autocomplete({
 		source: suggest_url,
		delay: 300,
 		autoFocus: false,
 		minLength: 0,
 		select: function( event, ui )
 		{
			$( "#search_major_code" ).val(ui.item.label);
			$( "#search_major_id" ).val(ui.item.label_id);
 			// do_search(true);
 		}
	});
}
enable_codeMajor.enabled=false;



// end my view search ditecso
